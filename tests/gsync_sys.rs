    ...
    unsafe {
        ...
        match status_result(status) {
            ...
            Err(e) => {
                ...
                println!(
                    "EnumSyncDevices: {:?} (this is normal if no GSync hardware is present)",
                    e
                );
                match e {
        ...
        match status_result(status) {
            Ok(()) => {
                ...
                match status_result(status) {
                    Ok(()) => {
                        println!(
                            "QueryCapabilities: version=0x{:08x} (capabilities retrieved successfully)",
                            caps.version
                        );
                        Ok(())
                    }
        ...
        match status_result(status) {
            Ok(()) => {
                ...
                if status == sys::status::NVAPI_INSUFFICIENT_BUFFER {
                    println!(
                        "GetTopology: Requires {} GPUs, {} displays",
                        gpu_count, display_count
                    );

                    if gpu_count > 0 || display_count > 0 {
                        // Allocate proper buffers
                        let mut gpus: Vec<g::NV_GSYNC_GPU> =
                            vec![g::NV_GSYNC_GPU::zeroed(); gpu_count as usize];
                        let mut displays: Vec<g::NV_GSYNC_DISPLAY> =
                            vec![g::NV_GSYNC_DISPLAY::zeroed(); display_count as usize];
                        ...
                        match status_result(status2) {
                            Ok(()) => {
                                println!(
                                    "GetTopology: Successfully retrieved {} GPUs, {} displays",
                                    gpu_count, display_count
                                );
                                ...
                                for (i, gpu) in gpus.iter().enumerate().take(gpu_count as usize) {
                                    println!(
                                        "  GPU[{}]: handle valid={}",
                                        i,
                                        !gpu.hPhysicalGpu.is_null()
                                    );
                                }

                                for (i, display) in
                                    displays.iter().enumerate().take(display_count as usize)
                                {
                                    println!(
                                        "  Display[{}]: id={} masterable={} syncState={:?}",
                                        i,
                                        display.displayId,
                                        display.isMasterable,
                                        display.syncState
                                    );
                                }
                            ...
                            Err(e) => {
                                return Err(format!("GetTopology second call failed: {:?}", e));
                            }
                        }
                    } else {
                ...
                match status_result(status) {
                    Ok(()) => {
                        println!(
                            "GetTopology: {} GPUs, {} displays (direct call)",
                            gpu_count, display_count
                        );
                        Ok(())
                    }
        ...
        match status_result(status) {
            Ok(()) => {
                ...
                match status_result(status) {
                    Ok(()) => {
                        println!(
                            "GetControlParameters: version=0x{:08x} (parameters retrieved successfully)",
                            params.version
                        );
                        Ok(())
                    }
        ...
        match status_result(status) {
            Ok(()) => {
                ...
                match status_result(status) {
                    Ok(()) => {
                        println!(
                            "GetSyncStatus: synced={} stereoSynced={} signalAvailable={}",
                            sync_status.bIsSynced,
                            sync_status.bIsStereoSynced,
                            sync_status.bIsSyncSignalAvailable
                        );
                        Ok(())
                    }
    ...
    unsafe {
        ...
        match status_result(status) {
            Ok(()) => {
                ...
                let status = g::NvAPI_GSync_GetStatusParameters(
                    handles[0],
                    &mut params_v1 as *mut _ as *mut g::NV_GSYNC_STATUS_PARAMS,
                );
                match status_result(status) {
                    Ok(()) => {
                        println!(
                            "GetStatusParameters (V1): version=0x{:08x} (parameters retrieved successfully)",
                            params_v1.version
                        );
                        Ok(())
                    }
                    Err(e) => {
                        ...
                        let status = g::NvAPI_GSync_GetStatusParameters(
                            handles[0],
                            &mut params_v2 as *mut _ as *mut g::NV_GSYNC_STATUS_PARAMS,
                        );
                        match status_result(status) {
                            Ok(()) => {
                                println!(
                                    "GetStatusParameters (V2): version=0x{:08x} (parameters retrieved successfully)",
                                    params_v2.v1.version
                                );
                                Ok(())
                            }
                            Err(e2) => Err(format!(
                                "GetStatusParameters failed (V1: {:?}, V2: {:?})",
                                e, e2
                            )),
                        }
                    }
        ...
        match status_result(status) {
            Ok(()) => {
                ...
                match status_result(status) {
                    Ok(()) => {
                        println!(
                            "AdjustSyncDelay (SyncSkew): syncSteps={} (delay adjusted successfully)",
                            sync_steps
                        );
                        Ok(())
                    }
                    Err(e) => {
                        ...
                        match status_result(status) {
                            Ok(()) => {
                                println!(
                                    "AdjustSyncDelay (Startup): syncSteps={} (delay adjusted successfully)",
                                    sync_steps2
                                );
                                Ok(())
                            }
                            Err(e2) => Err(format!(
                                "AdjustSyncDelay failed (SyncSkew: {:?}, Startup: {:?})",
                                e, e2
                            )),
                        }
                    }
...
fn test_gsync_struct_sizes_and_versions() -> Result<(), String> {
    ...
    println!(
        "NV_GSYNC_CAPABILITIES_V1 size: {}",
        std::mem::size_of::<g::NV_GSYNC_CAPABILITIES_V1>()
    );
    println!(
        "NV_GSYNC_CAPABILITIES_V2 size: {}",
        std::mem::size_of::<g::NV_GSYNC_CAPABILITIES_V2>()
    );
    println!(
        "NV_GSYNC_CAPABILITIES_VER_1: 0x{:08x}",
        g::NV_GSYNC_CAPABILITIES_VER_1
    );
    println!(
        "NV_GSYNC_CAPABILITIES_VER_2: 0x{:08x}",
        g::NV_GSYNC_CAPABILITIES_VER_2
    );

    // Display and GPU structs
    println!(
        "NV_GSYNC_DISPLAY size: {}",
        std::mem::size_of::<g::NV_GSYNC_DISPLAY>()
    );
    println!(
        "NV_GSYNC_GPU size: {}",
        std::mem::size_of::<g::NV_GSYNC_GPU>()
    );
    ...
    // Control params
    println!(
        "NV_GSYNC_CONTROL_PARAMS size: {}",
        std::mem::size_of::<g::NV_GSYNC_CONTROL_PARAMS>()
    );
    println!(
        "NV_GSYNC_CONTROL_PARAMS_VER: 0x{:08x}",
        g::NV_GSYNC_CONTROL_PARAMS_VER
    );

    // Status structs
    println!(
        "NV_GSYNC_STATUS size: {}",
        std::mem::size_of::<g::NV_GSYNC_STATUS>()
    );
    println!(
        "NV_GSYNC_STATUS_PARAMS_V1 size: {}",
        std::mem::size_of::<g::NV_GSYNC_STATUS_PARAMS_V1>()
    );
    println!(
        "NV_GSYNC_STATUS_PARAMS_V2 size: {}",
        std::mem::size_of::<g::NV_GSYNC_STATUS_PARAMS_V2>()
    );
    println!("NV_GSYNC_STATUS_VER: 0x{:08x}", g::NV_GSYNC_STATUS_VER);
    println!(
        "NV_GSYNC_STATUS_PARAMS_VER_1: 0x{:08x}",
        g::NV_GSYNC_STATUS_PARAMS_VER_1
    );
    println!(
        "NV_GSYNC_STATUS_PARAMS_VER_2: 0x{:08x}",
        g::NV_GSYNC_STATUS_PARAMS_VER_2
    );

    // Delay struct
    println!(
        "NV_GSYNC_DELAY size: {}",
        std::mem::size_of::<g::NV_GSYNC_DELAY>()
    );
...
fn test_gsync_enum_creation() -> Result<(), String> {
    ...
    // Display sync states
    println!(
        "DisplaySyncState::Unsynced: {}",
        g::NVAPI_GSYNC_DISPLAY_SYNC_STATE_UNSYNCED
    );
    println!(
        "DisplaySyncState::Slave: {}",
        g::NVAPI_GSYNC_DISPLAY_SYNC_STATE_SLAVE
    );
    println!(
        "DisplaySyncState::Master: {}",
        g::NVAPI_GSYNC_DISPLAY_SYNC_STATE_MASTER
    );

    // Topology connectors
    println!(
        "TopologyConnector::None: {}",
        g::NVAPI_GSYNC_GPU_TOPOLOGY_CONNECTOR_NONE
    );
    println!(
        "TopologyConnector::Primary: {}",
        g::NVAPI_GSYNC_GPU_TOPOLOGY_CONNECTOR_PRIMARY
    );
    println!(
        "TopologyConnector::Secondary: {}",
        g::NVAPI_GSYNC_GPU_TOPOLOGY_CONNECTOR_SECONDARY
    );

    // Polarity
    println!(
        "Polarity::RisingEdge: {}",
        g::NVAPI_GSYNC_POLARITY_RISING_EDGE
    );
    println!(
        "Polarity::FallingEdge: {}",
        g::NVAPI_GSYNC_POLARITY_FALLING_EDGE
    );
    println!(
        "Polarity::BothEdges: {}",
        g::NVAPI_GSYNC_POLARITY_BOTH_EDGES
    );
    ...
    println!(
        "VideoMode::NtscPalCam: {}",
        g::NVAPI_GSYNC_VIDEO_MODE_NTSCPALSECAM
    );

    // Sync sources
    println!("SyncSource::VSync: {}", g::NVAPI_GSYNC_SYNC_SOURCE_VSYNC);
    println!(
        "SyncSource::HouseSync: {}",
        g::NVAPI_GSYNC_SYNC_SOURCE_HOUSESYNC
    );
