...
use nvapi::sys::gsync::{
    DisplaySyncState, NVAPI_GSYNC_POLARITY, NVAPI_GSYNC_SYNC_SOURCE, NVAPI_GSYNC_VIDEO_MODE,
};
use nvapi::{GSyncDevice, PhysicalGpu, initialize};
                ...
                match device.get_topology() {
                    Ok((gpus, displays)) => {
                        ...
                        match device.get_physical_gpus() {
                            Ok(phys_gpus) => {
                                println!(
                                    "  Found {} physical GPUs connected to GSync device",
                                    phys_gpus.len()
                                );
...
fn test_gsync_driver_version_info() -> Result<(), String> {
    ...
    let gpus =
        PhysicalGpu::enumerate().map_err(|e| format!("Failed to enumerate GPUs: {:?}", e))?;
    if !gpus.is_empty() {
        let gpu = &gpus[0];
...
fn test_gsync_get_topology() -> Result<(), String> {
    ...
    let device = &devices[0];
    let (gpus, displays) = device
        .get_topology()
        .map_err(|e| format!("Failed to get topology: {:?}", e))?;

    println!(
        "GetTopology: {} GPUs, {} displays",
        gpus.len(),
        displays.len()
    );
...
fn test_gsync_adjust_sync_delay() -> Result<(), String> {
    ...
    match device.adjust_sync_delay(g::NVAPI_GSYNC_DELAY_TYPE_SYNC_SKEW, &mut delay) {
        Ok(steps) => {
            ...
            println!(
                "  Adjusted delay: lines={} pixels={}",
                delay.numLines, delay.numPixels
            );
            Ok(())
        }
        Err(e) => {
            ...
            match device.adjust_sync_delay(g::NVAPI_GSYNC_DELAY_TYPE_STARTUP, &mut delay2) {
                Ok(steps) => {
                    ...
                    println!(
                        "  Adjusted delay: lines={} pixels={}",
                        delay2.numLines, delay2.numPixels
                    );
                    Ok(())
                }
                Err(e2) => Err(format!(
...
fn test_gsync_get_physical_gpus() -> Result<(), String> {
    ...
    println!(
        "GetPhysicalGpus: {} GPUs connected to GSync device",
        gpus.len()
    );
...
fn test_gsync_set_sync_state_settings() -> Result<(), String> {
    ...
    println!(
        "SetSyncStateSettings: Testing with {} displays",
        displays.len()
    );
        ...
        match DisplaySyncState::from_raw(display.syncState) {
            Ok(current_state) => {
                ...
                println!(
                    ...
                    display.displayId, current_state
                );
            }
            Err(e) => {
                println!(
                    "  Display {}: invalid sync state ({:?})",
                    display.displayId, e
                );
    ...
    match device.set_sync_state_settings(sync_settings, 0) {
        ...
        Err(e) => {
            ...
            println!(
                "SetSyncStateSettings: Failed ({:?}) - this may be expected",
                e
            );
...
fn test_gsync_set_control_parameters() -> Result<(), String> {
    ...
    match device.set_control_parameters(&mut params) {
        ...
        Err(e) => {
            ...
            println!(
                "SetControlParameters: Failed ({:?}) - this may be expected without proper hardware/permissions",
                e
            );
            // Don't fail the test - this is expected in many scenarios
            Ok(())
